...
get_args <- function(){
	...
	# return parser$parse_args(c("results/penicillin_maps.tsv", "results/penicillin_gene_info.tsv"));
}

main <- function(args){

	args$path_maps %>% read_tsv() -> dat_all;

	message(nrow(dat_all));
	if(nrow(dat_all) == 0){
		tibble(`no genes mapped` = numeric()) -> dat_empty;
		dat_empty %>% write_tsv(paste0(args$prefix, "_genes.tsv"));
		dat_empty %>% write_tsv(paste0(args$prefix, "_genes_near.tsv"));
		return();
	}
